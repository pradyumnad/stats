...
public class Stats {
	/**
	 * Z variables to calculate the quantiles 
	 */
	...
	public static class Map extends Mapper<LongWritable, Text, Text, MapWritable> {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			mw.put(diffKey, new IntWritable(number));
			
			context.write(textKey, mw);
		}
	}
	
	public static class Combine extends Reducer<Text, MapWritable, Text, MapWritable> {
		Text countKey = new Text("count");
		Text maxKey = new Text("max");
		Text minKey = new Text("min");
		Text sumKey = new Text("sum");
		...
		protected void reduce(Text key, Iterable<MapWritable> values,
			...
			int sum = ((IntWritable)firstMapWritable.get(sumKey)).get();
			...
			mw.put(countKey, new IntWritable(count));
			mw.put(maxKey, new IntWritable(max));
			mw.put(minKey, new IntWritable(min));
			mw.put(sumKey, new IntWritable(sum));
	...
	public static class Reduce extends Reducer<Text, MapWritable, Text, FloatWritable> {
		...
		protected void reduce(Text key, Iterable<MapWritable> values,
			...
			for(MapWritable m : values) {
				...
				IntWritable countIntWritable = (IntWritable) m.get(countKey);
				// temp helpers
				delta = number - mean; // a.mean - b.mean
				mean = mean + delta/count;
				M2 += delta * (number - mean);
			...
			context.write(new Text("25th"), new FloatWritable((float) twentyfifth));
			context.write(new Text("50th"), new FloatWritable((float) fiftyth));
			context.write(new Text("75th"), new FloatWritable((float) seventyfifth));
		}
	}
	/**
	 * @param args
	...
	public static void main(String[] args) throws IllegalArgumentException, IOException, ClassNotFoundException, InterruptedException {
		...
		Job job = new Job(conf, "stats-tmp");
